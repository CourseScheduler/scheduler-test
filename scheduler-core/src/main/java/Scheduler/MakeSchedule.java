...
public class MakeSchedule extends JPanel implements Tab {
	...
	*********************************************************/
	public ClassPanel getInfoPanel(Section section){
		...
		if (section != null){
			inner.add(new JLabel(Integer.toString(section.getCRN()), SwingConstants.CENTER));
			inner.add(new JLabel("<html><strong>" + section.getCourseID() + " " + section.getSection() + "</strong></html>", SwingConstants.CENTER));
			inner.add(new JLabel("<html><strong>" + section.getTitle() + "</strong></html>", SwingConstants.CENTER));
			if (section.getNotes().length() != 0){
				inner.add(new JLabel("<html><font color=\"red\">" + section.getNotes(), SwingConstants.CENTER));
			}
			inner.add(new JLabel("Course Rating: " + Double.toString(Math.round(section.getRating() * 10)/10.0), SwingConstants.CENTER));
			inner.add(new JLabel(section.getInstructor().getName(), SwingConstants.CENTER));
			if (!section.getInstructor().getName().equals("STAFF") && section.getInstructor().getRating() != 0.0){
				inner.add(new JLabel("Professor Rating: " + Double.toString(Math.round(section.getInstructor().getRating() * 10)/10.0), SwingConstants.CENTER));
			}
			inner.add(new JLabel(section.getCredit() + " Credits", SwingConstants.CENTER));
			inner.add(new JLabel(section.getDaysStr() + " " + section.getPeriodStr(), SwingConstants.CENTER));
			inner.add(new JLabel(section.getLocation(), SwingConstants.CENTER));
			
			JLabel seats = new JLabel(section.isClosed() ? "Course Closed" : "Open Seats: " + section.getSeats(), SwingConstants.CENTER);
			seats.setForeground(section.isClosed() ? Color.RED : seats.getForeground());
			inner.add(seats);
			
			if(section.hasSecondary()){
				inner.add(new JLabel(section.getSecDaysStr() + " " + section.getSecPeriodStr(), SwingConstants.CENTER));
				inner.add(new JLabel(section.getSecLocation(), SwingConstants.CENTER));
			}
		}
		else {
	...
	*********************************************************/
	...
	public class ConflictPanel extends JPanel{
		...
		public ConflictPanel(Section conf, Section miss, Section in){
			...
			sixth.setHorizontalAlignment(SwingConstants.CENTER);
